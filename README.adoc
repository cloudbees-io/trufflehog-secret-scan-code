...
Note:  If only-verified is set to false, Trufflelog may utilize an external third-party tool to scan unverified results. 
Customers concerned about security implications involving third-party tools should set the only-verified value to true.
...
To scan a repository branch with only-verified set false :
