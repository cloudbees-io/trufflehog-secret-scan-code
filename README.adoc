...
This action provides flexibility by allowing users to scan a local codebase or else provide a URL to scan a remote repository.
...
To scan a repository branch with `only-verified` set to false, which may return false positives:
